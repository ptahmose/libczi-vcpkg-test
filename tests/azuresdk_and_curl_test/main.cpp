...
int main() 
{
	...
	bool azure_blob_input_stream_found = false;
	...
	for (int i = 0; i < stream_object_count; ++i)
	{
		...
		if (stream_class_info.class_name == "azure_blob_inputstream")
		{
			azure_blob_input_stream_found = true;
		}
		else if (stream_class_info.class_name == "curl_http_inputstream")
		{
	...
	return (azure_blob_input_stream_found && curl_input_stream_found) ? EXIT_SUCCESS : EXIT_FAILURE;
}